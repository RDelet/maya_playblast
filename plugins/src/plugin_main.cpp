...
MStatus initializePlugin(MObject obj)
{
    ...
    const MStatus status = plugin.registerCommand(
        ReadPixelsCmd::kCommandName,
        ReadPixelsCmd::creator,
        ReadPixelsCmd::newSyntax
    );
    if (status)
...
{
    // Détruit le unique_ptr et désenregistre l'override via OverrideDeleter
